...
def early_stopping(current_psnr, recent_psnrs, 
                   variance_threshold=1e-4, decline_threshold=0.08, 
                   window_size=20, patience=20, mean_gap=8):
    """
    ...
    # Add the current PSNR to the list
    recent_psnrs.append(current_psnr)
    
    # Maintain the size of recent_psnrs
    if len(recent_psnrs) > window_size:
        recent_psnrs.pop(0)

    # Check for variance in recent PSNR values
    if len(recent_psnrs) == window_size:
        psnr_variance = np.var(recent_psnrs)
        if psnr_variance < variance_threshold:
            print("Early stopping due to low PSNR variance.")
            return True

    # Check for decline in PSNR trend
    if len(recent_psnrs) >= patience + mean_gap:
        recent_mean = np.mean(recent_psnrs[-patience:])
        previous_mean = np.mean(recent_psnrs[-(patience + mean_gap):-mean_gap])
        if recent_mean < previous_mean - decline_threshold:
            print("Early stopping due to declining PSNR trend.")
            return True
